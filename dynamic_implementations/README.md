The `dynamic_implementations` folder is intended currently to run the code in dynamic outlets, like discord.

Ideally, we can push some of these into the maeser package, but there are a few things in the way at the moment:
- The `generate_repsonse.py` could be moved, but requires things like keys and tokens to be defined. It currently gets those from the `config.yaml` in this folder, but may be set up as variables that are defined in another way.
...
- `bot_data` is accessed by the handlers and the `generate_response.py`. This folder would need to stay outside the package, and is defined by administrators. This can probably move into the web_client folder, but would require some refactoring.
