...
pipeline_prompt: str = """You are speaking from the perspective of being very knowledgable in the subject at hand.
    You will answer a question about the topic that is most relevant to the question.
    ...
    If the question is unrelated to the subject entirely, redirect the user to the original topic, unless you think it useful to discuss the tangent.
    

    {context}
"""
from maeser.graphs.simple_rag import get_simple_rag
# Pipeline RAG not mentioned in documentation
...
vectorstore_config = {
    "General Knowledge": f"{VEC_STORE_PATH}/general",
    "Star Wars Vehicles": f"{VEC_STORE_PATH}/vehicles",
    "Star Wars Characters": f"{VEC_STORE_PATH}/characters"
}

byu_maeser_pipeline_rag: CompiledGraph = get_pipeline_rag(
    vectorstore_config=vectorstore_config, 
    memory_filepath=f"{VEC_STORE_PATH}/pipeline_memory.db",
    # Make sure to include this in documentation
    api_key=OPENAI_API_KEY, 
    system_prompt_text=(pipeline_prompt),
    model=LLM_MODEL_NAME)
  
sessions_manager.register_branch(branch_name="pipeline", branch_label="Let's Start Learning!", graph=byu_maeser_pipeline_rag)
